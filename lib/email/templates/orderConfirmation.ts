...
export function renderOrderConfirmationEmail(data: OrderConfirmationEmailData): string {
  ...
  // Показываем все промокоды если их несколько
  const allCodes = data.allEntryCodes && data.allEntryCodes.length > 0 
    ? data.allEntryCodes 
    : (data.entryCode ? [data.entryCode] : [])
  
  const codesCount = allCodes.length
  
  const promoEntryHtml = data.participatesInPromo && codesCount > 0
    ? `
    <div style="background: linear-gradient(135deg, #ff8534 0%, #ff6b35 100%); border-radius: 16px; padding: 30px; margin: 30px 0; text-align: center;">
      ...
      <p style="margin: 0 0 20px 0; font-size: 16px; color: rgba(255,255,255,0.9);">
        ${codesCount === 1 ? 'Ваш код участия' : `Ваши ${codesCount} кодов участия`} в розыгрыше iPhone:
      </p>
      <div style="background: rgba(0,0,0,0.2); border-radius: 12px; padding: 15px; margin: 0 auto; display: inline-block;">
        ${allCodes.map((code, idx) => `
          <div style="font-size: ${codesCount > 3 ? '20px' : '28px'}; font-weight: 700; color: #ffffff; font-family: 'Courier New', monospace; letter-spacing: 2px; ${idx > 0 ? 'margin-top: 10px; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.2);' : ''}">
            ${code}
          </div>
        `).join('')}
      </div>
      <p style="margin: 20px 0 0 0; font-size: 14px; color: rgba(255,255,255,0.8);">
        Сохраните ${codesCount === 1 ? 'этот код' : 'эти коды'} — ${codesCount === 1 ? 'он понадобится' : 'они понадобятся'} при розыгрыше!
      </p>
...
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      <p>Благодарим за покупку в <strong>GETNWIN</strong>. Ваш заказ успешно оплачен и передан в обработку.</p>
      ...
      <div style="background: rgba(255, 133, 52, 0.1); border: 1px solid rgba(255, 133, 52, 0.3); border-radius: 12px; padding: 20px; margin: 30px 0;">
        <p style="margin: 0; font-size: 14px; color: #cccccc;">
          ...
          3. Доставка СДЭК занимает 3-7 дней<br>
          4. Вы получите уведомление о прибытии посылки
        </p>
      </div>

      <div style="background: linear-gradient(135deg, #0088cc 0%, #00a8e8 100%); border-radius: 12px; padding: 20px; margin: 30px 0; text-align: center;">
        ...
        <p style="margin: 0 0 15px 0; font-size: 14px; color: rgba(255,255,255,0.9);">
          Новости, акции и эксклюзивные предложения
        </p>
        <a href="https://t.me/+NFNJFoql6xplNzRi" target="_blank" style="display: inline-block; background: #ffffff; color: #0088cc; padding: 12px 30px; border-radius: 8px; font-weight: 600; text-decoration: none; font-size: 16px;">
          Перейти в канал →
        </a>
      </div>

      <p style="font-size: 14px; color: #999;">
        По всем вопросам пишите нам на <a href="mailto:support@getnwin.ru" style="color: #ff8534; text-decoration: none;">support@getnwin.ru</a>
      </p>
    </div>

    <div class="footer">
      <p>
        <a href="https://t.me/+NFNJFoql6xplNzRi">Telegram</a> •
        <a href="https://getnwin.ru/account">Мой аккаунт</a> •
        <a href="https://getnwin.ru/docs/offer">Публичная оферта</a> •
        <a href="https://getnwin.ru/docs/privacy">Политика конфиденциальности</a>
      </p>
...
export function renderOrderConfirmationEmailText(data: OrderConfirmationEmailData): string {
  ...
  const promoText = data.participatesInPromo && codesCount > 0
    ? `

🎉 ВЫ УЧАСТВУЕТЕ В РОЗЫГРЫШЕ!

${codesCount === 1 ? 'Ваш код участия:' : `Ваши ${codesCount} кодов участия:`}
${allCodes.map((code, idx) => `${idx + 1}. ${code}`).join('\n')}
...
Благодарим за покупку в GETNWIN. Ваш заказ успешно оплачен и передан в обработку.
${promoText}
СОСТАВ ЗАКАЗА:
${itemsList}
...
3. Доставка СДЭК занимает 3-7 дней
4. Вы получите уведомление о прибытии посылки

📱 ПОДПИСЫВАЙТЕСЬ НА НАШ TELEGRAM-КАНАЛ!
Новости, акции и эксклюзивные предложения: https://t.me/+NFNJFoql6xplNzRi

По всем вопросам пишите нам на support@getnwin.ru

---
Перейти в личный кабинет: https://getnwin.ru/account
